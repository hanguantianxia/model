...
class Decoder(object):
    ...
    def lm_task(self, lm_label_arr, lm_pos_mask, lm_pos_len, name='LM_task'):
        ...
        tgt_output = fluid.layers.softmax(tgt_output, use_cudnn=USE_CUDA,name=name+"softmax")
    ...
    def pretrain(self, goal_type_pos, goal_type_label,
                 lm_label_arr, lm_pos_mask, lm_pos_len):
        """
...
def fine_tunning():
    ...
    with fluid.program_guard(train_prog, startup_prog):
        ...
        decode = Decoder(token_ids, pos_ids, segment_ids, enc_slf_attn,
                         config=config, enc_input=enc_input, enc_input_mask=enc_mask)
    ...
    for epoch_id in range(EPOCH_NUM):
        ...
        data_gen = dataset.generate_dec_batch(batch_size=BATCH_SIZE)
        for batch_id, item in enumerate(data_gen):

            token_id_arr, segment_id_arr, pos_arr, slf_attn_mask_arr, \
            lm_mask_arr, lm_len_arr, lm_lable_arr, \
            goal_pos, goal_type_list = item
